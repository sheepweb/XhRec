...
fun JsonElement.BooleanOrElse(name: String, e: Boolean): Boolean {
    return this.jsonObject[name]?.jsonPrimitive?.boolean ?: e
}
...
fun JsonElement.PathSingleOrNull(jsonPath: String): JsonElement? {
    return JsonPath.path(jsonPath).getAll(this).singleOrNull()
}